...
function allStorage() {
    ...
    for (j = 0; j < values.length; j++) {
        $(".previousList").prepend("<button class='prev-city mt-1'>" + values[j] + "</button>");
    }
}
allStorage();
...
//This collection of current weather data and converts info into what is displayed.
    $.ajax({
        ...
        $(".previousList").prepend("<button class='prev-city mt-1'>" + subject + "</button>");
        localStorage.setItem(subject, subject);
        $(".boxEl").show();
        $(".forecastDashboard").show();
        ...
        for (i = 0; i < forecastTimes.length; i++) {
            if (forecastTimes[i].dt_txt[12] === "2") {
                ...
                if (forecastdisplay === false || forecastdisplay === undefined) {
                    $(".forecastList").append("<div class='my-3 pb-3 col-md-2 col-lg-2 forecast-day'>" +
